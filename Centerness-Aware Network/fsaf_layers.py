...
import math
from layers import build_centerness_target


def get_centermask_graph(inputs, begin_count):
    batch_centermask_pred = tf.tile(inputs[0], [1, 1, 256])
    batch_centermask_pred = batch_centermask_pred[:, begin_count:begin_count+tf.shape(inputs[1])[1]*tf.shape(inputs[1])[2], :]
    batch_centermask_pred = tf.reshape(batch_centermask_pred, [-1, tf.shape(inputs[1])[1], tf.shape(inputs[1])[2], 256])
    return inputs[1] * pow(math.e, batch_centermask_pred)
...
def level_select(cls_pred, regr_pred, gt_boxes, feature_shapes, strides, pos_scale=0.2):
    ...
    padding_gt_box_levels = (-1) * tf.ones((MAX_NUM_GT_BOXES - num_gt_boxes), dtype=tf.int64)
...
def build_CAM_target(gt_box_levels, gt_boxes, feature_shapes, num_classes, strides, pos_scale, ignore_scale):
    # Calculate centerness target
    center_target, center_mask = build_centerness_target(gt_box_levels, gt_boxes, feature_shapes, strides)
    gt_labels = tf.cast(gt_boxes[:, 4], tf.int32)
    gt_boxes = gt_boxes[:, :4]
    ...
    return [cls_target, cls_mask, cls_num_pos, regr_target, regr_mask,center_target,center_mask]


class CAMTarget(Layer):
    def __init__(self, num_classes,**kwargs):
        super(CAMTarget, self).__init__(**kwargs)
    ...
    def call(self, inputs, **kwargs):
        ...
        def _build_CAM_target(args):
            gt_box_levels = args[0]
            gt_boxes = args[1]
            return build_CAM_target(
                gt_box_levels,
                gt_boxes,
                feature_shapes=feature_shapes,
        ...
        outputs = tf.map_fn(
            ...
            dtype=[tf.float32, tf.bool, tf.float32, tf.float32, tf.bool, tf.float32, tf.bool],
        )
        return outputs
    ...
    def compute_output_shape(self, input_shape):
        ...
        return [[batch_size, None, self.num_classes], [batch_size, None], [batch_size, ], [batch_size, None, 4],
                [batch_size, None], [batch_size, None, 1], [batch_size, None]]
    ...
    def get_config(self):
        ...
        Returns
            Dictionary containing the parameters of this layer.
        """
        config = super(CAMTarget, self).get_config()
