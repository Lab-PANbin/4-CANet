...
from models.retinanet import CAM_predict_box
...
def create_models(backbone_retinanet, num_classes, weights, num_gpus=0, freeze_backbone=False, lr=1e-5, config=None):
    ...
    prediction_model = CAM_predict_box(model=model)
    ...
    training_model.compile(
        loss={
            ...
            'center_loss': lambda y_true, y_pred: y_pred,
        },
...
def create_callbacks(model, training_model, prediction_model, validation_generator, args):
    ...
    if args.evaluation and validation_generator:
        if args.dataset_type == 'pascal':
            ...
            raise ValueError('\'{}\' is invalid dataset type'.format(args.dataset_type))
...
def parse_args(args):
    ...
    # group.add_argument('--snapshot', help='Resume training from a snapshot.',default='snapshots/2020-12-07/resnet101_pascal_09.h5',type=str)
    ...
    parser.add_argument('--steps', help='Number of steps per epoch.', type=int, default=456)
...
def main(args=None):
    ...
    if args.snapshot is not None:
        ...
        model = model_with_weights(backbone.CANet(train_generator.num_classes(),
                                                 modifier=None),
                                   weights=args.snapshot, skip_mismatch=True)
        training_model = model
        prediction_model = CAM_predict_box(model=model)
        ...
        model, training_model, prediction_model = create_models(
            backbone_retinanet=backbone.CANet,
