...
import sys
from fsaf_layers import RegressBoxes, Locations, LevelSelect, CAMTarget
import losses
import initializers
import keras
...
def convert_model(model, nms=True, class_specific_filter=True):
    ...
    Raises
        ...
        ValueError: In case of an invalid savefile.
    """
    from .retinanet import CAM_predict_box
    return CAM_predict_box(model=model, nms=nms, class_specific_filter=class_specific_filter)
...
def assert_training_model(model):
    ...
    assert (all(output in model.output_names for output in
                ['cls_loss', 'regr_loss', 'center_loss', 'CAM_regression', 'CAM_classification', 'CAM_centernessmask'])), \
